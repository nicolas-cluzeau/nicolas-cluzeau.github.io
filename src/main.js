...
const projectData = {
  1: {
    ...
    fullContent: `
      ...
      <p>J’ai ensuite mis en place des règles de filtrage sur le pare-feu. Ces dernières permettaient d’autoriser ou bloquer certains flux transitant par le pare-feu. Cela ajoutait une couche de sécurité entre notre infrastructure et le réseau de l’école. J’ai mis en place le principe du moindre privilège, en ne cherchant qu’à ouvrir les flux nécessaires et bloquer tous les autres. Par exemple, j’ai bloqué les flux entre le WAN, les hyperviseurs et le VCenter, pour éviter que l’on puisse prendre la main sur ces derniers et causer une interruption de service.  Les seuls flux ouverts étaient les flux vers la machine hébergeant les conteneurs.</p>
      <p>Une fois l’infrastructure mise en place, je me suis attelé à la partie hébergement des conteneurs Docker. J’ai créé une machine Debian 11 à partir de l’interface VCenter, puis j’ai installé docker et docker-compose. Docker est une plateforme logicielle permettant de faire tourner certains applicatifs sous forme de conteneurs isolés. Comparé à une machine virtuelle, les conteneurs Docker sont plus efficients au niveau de l’utilisation des ressources de calcul car ils n’incluent pas de système d’exploitation, donc ils sont plus légers. Docker-compose est un outil additionnel de Docker, qui permet la création de plusieurs conteneurs en simultané à partir d’un fichier .yaml. Ce dernier permet de construire des infrastructures conteneurisées à partir d’un seul fichier et une seule commande.</p>
      <p>Une fois Docker et docker-compose installés, j’ai participé à la mise en place d’un autre outil : Portainer. C’est un outil permettant de centraliser la gestion de conteneurs Docker ou Kubernetes dans une interface web. Pour ce projet, il nous permettait de gérer des volumes ou réseaux sans aucune commande, et d’apporter un élément de gestion des conteneurs graphique lors des démonstrations de l’infrastructure. Une fois le conteneur Portainer créé, j’ai créé un compte personnel pour chacun, afin de pouvoir travailler à plusieurs depuis l’interface. Nous nous sommes ensuite répartis pour créer plusieurs conteneurs de différents types. Une stack LAMP (Linux, Apache, MySQL, PHP) a été mise en place, avec un conteneur Apache servant de site web et un conteneur MySQL fournissant la base de données. Nous souhaitions que cette stack se crée en même temps que le conteneur Portainer, dons je l’ai ajoutée dans le fichier .yaml utilisé par docker-compose.</p>
      ...
      <p>Une évolution avancée de cette infrastructure aurait commencé par la mise en cluster du PFSense, qui était seul. L’infrastructure aurait aussi été segmentée grâce à des VLAN, avec la passerelle de chaque sous-réseau qui serait portée par le PFSense. Côté conteneurisation, une bascule vers un orchestrateur tel que Kubernetes serait préférable pour gérer plus efficacement la scalabilité, ainsi qu’une implémentation d’une stack de supervision pour surveiller l’état de l’infrastructure, tout cela configuré via un outil d’automatisation tel qu’Ansible ou Terraform pour appliquer rapidement des changements ou reconstruire l’infrastructure.</p>
  ...
  2: {
    ...
    fullContent: `
      ...
      <h3>Étapes du projet</h3>
      <p>Une fois l’avant-projet validé, l’équipe s’est réunie afin de réaliser un schéma réseau de notre infrastructure. Nous avons souhaité passer du temps pour le réaliser afin que tous les choix techniques soient mûrement réfléchis. Nous nous sommes basés sur les expériences de chaque membre pendant notre alternance afin d’émuler au mieux l’infrastructure web d’une petite entreprise. Une information qui nous a été fournie par le commanditaire était que le moyen d’installation le plus utilisé par les utilisateurs de Bunkerweb est l’installation via Docker et docker-compose. Nous nous sommes donc logiquement orientés vers une infrastructure complètement conteneurisée.</p>
      <p>L'infrastructure était séparée en trois parties :</p>
      ...
      <ul>
        ...
        <li>Une partie pare-feu, avec tous les conteneurs nécessaires au bon fonctionnement de Bunkerweb.</li>
      </ul>

      <p>De mon côté, j’ai pu me connecter en SSH à la machine Ubuntu fournie par le commanditaire, avant d’installer Docker et docker-compose. J’ai ensuite exploré la documentation de Bunkerweb afin de faire un choix entre les trois installations possibles : Docker, Docker Autoconf, et Docker Swarm. La différence entre Docker et Docker Autoconf est que, dans le mode Autoconf, un conteneur bw-autoconf est ajouté. Ce dernier permet d’appliquer des configurations sur des services tels que des applications web à partir de labels ajoutés dans le fichier docker-compose. Le commanditaire avait aussi recommandé de ne pas utiliser l’installation Docker Swarm, car elle n’était que très rarement utilisée. J’ai donc choisi l’installation Docker Autoconf.</p>
      <p>J’ai utilisé le fichier docker-compose fourni dans la documentation de Bunkerweb pour vérifier l’installation de Docker et docker-compose, ainsi que pour me familiariser avec la structure du pare-feu avant d’ajouter des fonctionnalités. Ce dernier est composé de plusieurs conteneurs :</p>
      ...
      <p>Une fois la première installation de Bunkerweb validée, j’ai pu utiliser une licence payante PRO fournie par le commanditaire. Cela a permis d’ajouter au fur et à mesure toutes les fonctionnalités de sécurité souhaitées. Un CAPTCHA a été mis en place afin de bloquer l’accès à l’interface web aux robots présents sur Internet. La création d’un certificat SSL autosigné a été implémentée, qui fut remplacée par l’utilisation d’un module Let’s Encrypt intégré plus tard dans le projet, ainsi qu’un proxy inversé afin de ne pas exposer directement les adresses IP des futurs serveurs web. La mise en place d’une protection anti-DDoS et d’un compte administrateur a aussi été incluse dans l’autoconfiguration.</p>
      <p>Après la mise en place de Bunkerweb, je me suis attelé à la création de la stack de supervision. Nous avions l’avantage qu’une partie de l’équipe avait réalisé un projet informatique en semestre 4 qui incluait une infrastructure de supervision conteneurisée. J’ai pu réutiliser les technologies mises en place dans ce projet, afin de créer une stack de supervision permettant de récupérer toutes les métriques de l’infrastructure docker globale, tout en étant protégée par le pare-feu Bunkerweb. La supervision est assurée par plusieurs conteneurs :</p>
      ...
      <p>J’ai mis en place cette stack de supervision en local, sur des machines virtuelles avec Bunkerweb installé. Cela permettait d’adapter la configuration des conteneurs à Bunkerweb sans causer de possible incident de production sur la machine fournie par le commanditaire. Une fois la configuration stable et fonctionnelle, j’ai ajouté la stack de supervision au fichier docker-compose existant. J’ai validé cette partie en me connectant à l’interface web de Grafana, en créant un dashboard permettant de visualiser les métriques des conteneurs telles que la consommation de processeur, de mémoire et de stockage, puis en notant que les métriques étaient bien collectées et affichées. Cependant, le stockage de toutes ces métriques consommait beaucoup de stockage sur la machine hôte que prévu, saturant le disque au bout d’une semaine. Nous avions dû écrire un e-mail à notre commanditaire afin d’obtenir une extension de disque importante, et de limiter notre rétention des métriques à trente jours maximum pour garder un historique sans saturer le stockage à nouveau.</p>
      <p>En parallèle de la supervision, j’ai commencé la mise en place d’une série de serveurs web. Puisque le rôle principal d’un WAF est de protéger des installations web, le but était de mettre en place plusieurs serveurs web, avec des niveaux de protections variés, afin de vérifier la résistance du pare-feu aux attaques de type DDoS et XSS (Cross-Side Scripting). Avec des camarades, nous avons décidé d’utiliser deux conteneurs par stack web, un serveur web Apache2 et une base de données PostgreSQL contenant un jeu de données fictif. Les stacks web mises en place étaient les suivantes :</p>
      ...
      <p>Comme pour la supervision, la création des stacks web s’est réalisée en local, pour éviter tout incident sur la machine fournie par le commanditaire. Une fois les serveurs prêts, j’ai ajouté le fichier docker-compose créé pour ces derniers au fichier docker-compose existant. Afin de gérer les quatre bases de données de manière simple, j’ai ajouté un conteneur PgAdmin, qui est un outil d’administration des bases de données PostgreSQL via une interface web. J’ai vérifié l’installation en me connectant à chaque serveur depuis mon poste de travail, puis en demandant à un camarade de faire de même. Pour terminer, j’ai dû modifier le dashboard Grafana en y ajoutant les nouveaux conteneurs créés.</p>
      <p>La dernière partie était la réalisation de tests de résistance et d’analyses de vulnérabilités. L’équipe entière s’est particulièrement concentrée sur les tests de résistance, puisque nous savions que nos connaissances en termes de pentesting et de chasse aux vulnérabilités étaient limitées. De plus, le commanditaire nous avait informé à la fin du deuxième sprint qu’il souhait plutôt que nous nous focalisions sur les tests de résistance, ainsi que sur la réalisation d’un retour d’expérience utilisateur.</p>
      <p>Nous nous sommes réunis pour débattre quel outil utiliser, et nous avons choisi l’outil OWASP ZAP afin de réaliser nos tests. C’est un scanner de sécurité d’application web open-source développé par l’OWASP (Open Web Application Security Project), qui est une communauté travaillant sur la sécurité des applications web. Nous l’avons choisi puisqu’il nous permettait de centraliser tous nos tests sur un seul outil. Avec OWASP ZAP, j’ai pu tester la protection anti-DDoS sur toutes les pages web accessibles, c’est-à-dire la page d’administration de Bunkerweb, l’interface web de Grafana, et les quatre sites web. Sur les sites web, j’ai aussi testé la protection contre les attaques XSS, en essayant d’injecter du code JavaScript dans un formulaire.</p>
      <p>J’ai finalement rédigé un rapport complet du projet, détaillant la mise en place de l’infrastructure de test, les différents labels utilisés pour activer les fonctionnalités choisies dans l’autoconf, les résultats des tests de résistance, ainsi que des recommandations pour les développeurs de Bunkerweb. Concernant les résultats, la protection anti-DDoS s’est révélée très efficace, bloquant toutes les tentatives d’attaque en bannissant l’IP publique de l’attaquant pendant vingt-quatre heures. Il a été cependant trouvé trop sensible, car je me suis retrouvé plusieurs fois banni en changeant trop rapidement de page lors de la configuration de Bunkerweb. Il a donc recommandé aux développeurs d’ajouter une option permettant de réduire la sensibilité de l’anti-DDoS. Une autre remarque était que la documentation de Bunkerweb était plutôt légère, et cela m’avait causé une certaine perte de temps lors de la première configuration, ce que j’ai mentionné dans le rapport.</p>
      ...
      <p>J’ai cependant pu réutiliser la partie supervision de l’infrastructure lors d’un mini-projet pendant mon alternance, en mettant en place une supervision Prometheus et un dashboard Grafana permettant d’obtenir des informations clés sur les consommations matérielles de certains clients sur une infrastructure virtualisée VMware VCenter.</p>
  ...
  5: {
    ...
    fullContent: `
      ...
      <p>C’est à la suite de ce POC que j’ai remplacé mon ancien maître de formation. Lors d’une réunion de préparation des déploiements de production, j’ai été assigné sur la tâche de création et d’organisation des playbooks avec l’équipe d’automatisation. Un playbook Ansible est un fichier .yaml qui permet de configurer le déploiement d’applications sur une ou plusieurs machines. Ici, j’ai utilisé Ansible afin de pousser la même configuration de l’agent sur les machines d’une même gamme de produits. Pour s’organiser sur les déploiements par gamme, j’ai utilisé un outil interne appelé VMOutput. C’est un fichier Excel généré tous les jours, contenant toutes les informations nécessaires sur les machines virtuelles présentes sur les infrastructures virtualisées VMware du groupe Berger-Levrault. Un export de ces informations via l’outil RVTools est combiné dans un script PowerShell à un fichier SharePoint rempli par les différentes Business Units du groupe. Certaines informations, telles qu’un identifiant de l’offre auquel la machine appartient, ou le nom du client sont ajoutées. Cela nous permet d’avoir un fichier combinant les configurations techniques des machines virtuelles, tout en ayant les informations des BU pour organiser les déploiements.</p>
      <p>J’ai ensuite planifié les déploiements un mois en avance. Cela laissait le temps de prévenir les différentes BU pour que les équipes puissent réaliser les communications aux clients. Le déploiement de l’agent ne causait pas de perturbations sur les activités clients lors du POC, mais les procédures générales de mon équipe demandaient tout de même de réaliser une communication afin de prévenir sur la durée d’intervention, les impacts possibles et les machines concernées. Les déploiements étaient organisés les lundis, mardis, et plus rarement les jeudis en début d’après-midi. Chacune de ces réunions servait au déploiement de l’agent OneAgent sur environ une vingtaine de machines maximum. Pour des gammes de produits plus petites, je les combinais pour gagner du temps. Pour les gammes les plus importantes, je réalisais les déploiements par lots, souvent triés par système d’exploitation.</p>
      <p>Dans l’ensemble, les déploiements, qui se sont déroulés de janvier à avril 2025, n’ont pas causé d’interruption majeure, sauf pour une gamme de produits. En février, lors du déploiement de l’agent sur des machines du produit ATAL, qui est un logiciel de gestion des services techniques pour les collectivités françaises, un incident s’est produit. Sur la douzaine de machines Windows Server 2016 du produit, cinq ne répondaient plus après l’installation de l’agent OneAgent. Après une demi-heure de troubleshooting, j’ai décidé de réaliser un rollback, afin de restaurer l’accès au produit pour les clients. Dans la foulée, j’ai été chargé de mener l’enquête de ces interruptions de service. J’ai cloné les machines incriminées et je les ai isolées derrière un pare-feu de remédiation. Ce pare-feu PFSense me permettait de garder la configuration réseau des machines, tout en évitant qu’elles communiquent avec l’extérieur. Je pouvais alors me connecter à elles via une console d’administration dédiée à cet effet, sans opérer de manipulation sur la production. J’ai ensuite essayé à plusieurs reprises de réinstaller l’agent OneAgent sur les machines manuellement, puis avec Ansible. À chaque fois, les machines ne répondaient plus, et certaines redevenaient disponibles après un redémarrage. Avec l’aide du support Dynatrace, j’ai pu extraire les logs système et de l’agent lors de l’installation, afin de déterminer la cause de l’incident.</p>
      <p>L’analyse du support a découvert que l’un des pilotes intégrés au fichier d’installation de l’agent, dont le rôle était de remonter les métriques de la carte réseau des machines virtuelles Windows, causait une erreur d’incompatibilité. Il m’a conseillé de tester une installation de l’agent avec une configuration choisissant un autre pilote, qui était utilisé dans des versions plus anciennes de OneAgent. Selon eux, il était possible que le pilote soit trop récent pour le système d’exploitation. Après un test sur toutes les machines clonées, l’installation s’est déroulée sans problème. La semaine suivante, un second déploiement a été organisé sur toutes les machines ATAL, en utilisant l’ancien pilote dans la configuration. Aucun problème n’avait été détecté, ce qui nous a permis de reprendre les déploiements sur les machines Windows Server 2016. Ce qui m’a surpris le plus, c’est que l’incident ne s’était pas déclaré pendant le POC, malgré le test sur des machines Windows Server 2016, ni sur le restant du parc informatique, alors que plus d’une centaine de machines utilisaient ce système d’exploitation.</p>
      ...
      <p>Environ un an après la conclusion du projet, la supervision Dynatrace est utilisée par toutes les Business Units client. Les équipes de support se servent de Dynatrace pour superviser les machines du côté matériel au côté applicatif, et qui permet de créer automatiquement des tickets en cas d’incident. Les managers peuvent créer des dashboards par gamme de produits afin de récupérer des informations clés à transmettre aux clients ainsi qu’au comité exécutif.</p>

      <h3>Regard critique</h3>
      <p>Ce projet était particulier puisque j’ai dû rejoindre l’équipe en cours de route. Cela m’a demandé de faire face à de nombreux challenges, notamment de m’intégrer dans un groupe composé de personnes de plusieurs équipes et réparties sur différents sites physiques. J’ai pu découvrir la suite Dynatrace et ses différents composants, ainsi que découvrir Ansible et dans la résolution d’incidents complexes. Je suis très heureux du résultat du projet et d’avoir pu assurer sa réussite.</p>
      `
  }
};
  ...
  humaines: {
    ...
    3: {
      ...
      fullContent: `
        ...
        <p>Lors du projet DockStock, j’ai pu travailler au sein d’un groupe de 5 personnes, dont un membre se situant en dehors du campus d’Agen. La collaboration était donc fondamentale pour réussir le projet. J’ai participé à la répartition des différentes tâches entre nous, tout en assurant ma capacité à pouvoir basculer sur les tâches de mes camarades pour les aider lorsqu’ils en avaient besoin, renforçant la cohésion de l’équipe. Des mini-réunions ont aussi été organisées pour partager les avancées de chacun et trouver des solutions aux problèmes découverts récemment. Ce projet m’a permis de renforcer ma capacité à écouter les autres, et m’a fait comprendre qu’il faut que tous les membres d’une équipe collaborent afin de mettre en place une infrastructure de cette complexité.</p>
        <p>Lors du projet Walltesters, j’ai pu travailler au sein d’un groupe de 9 personnes, ce qui était nouveau pour moi. J’ai aussi dû travailler en distanciel pendant au moins la moitié du projet. Cela a demandé une bien meilleure organisation que le projet DockStock. J’ai participé à l’organisation de réunions hebdomadaires en ligne, ainsi qu’à la gestion du tableau Trello de l’équipe. Je me suis aussi rendu aussi disponible que possible pour pouvoir aider mes camarades dans leurs tâches afin qu’aucun blocage ne reste trop longtemps. Pour des parties telles que les tests de résistance, j’ai dû me coordonner avec plusieurs camarades afin d’avoir quelqu’un qui regardait la supervision, une autre qui simulait un accès légitime, pendant que je lançais l’attaque. Grâce à ce projet, j’ai pu renforcer mes compétences acquises pendant le projet DockStock, et j’ai pu découvrir de nouvelles méthodes afin de m’organiser avec une équipe de grande taille.</p>
        <p>Lors du projet de déploiement d’un système de supervision, j’ai rejoint le projet alors qu’il avait déjà commencé, prenant la place de mon ancien maître de formation. Ce projet était bien plus complexe que les projets que j’ai pu faire à l’ESIEA, avec une collaboration entre plusieurs équipes de Berger-Levrault. Ici, une mauvaise collaboration avec ces dernières aurait pu avoir des conséquences importantes, puisque je travaillais sur des machines clientes. J’ai pu participer à des réunions hebdomadaires et des réunions de jalons organisées par l’équipe des chefs de projet. J’ai ensuite planifié à l’avance les déploiements de la solution OneAgent avec l’équipe d’automatisation et les différentes Business Units. Cette étape demandait une coordination parfaite, car une incompréhension lors du déploiement aurait pu causer une interruption de service. Lorsque des incidents techniques sont apparus, j’ai dû collaborer rapidement avec le support Dynatrace afin de trouver une solution et reprendre les déploiements sur une gamme de produits. Au final, notre collaboration efficace nous a permis de déployer l’agent OneAgent sur tous les applicatifs clients dans les délais, permettant de moderniser nos outils de supervision et améliorer notre vue globale du datacenter.</p>
...
const presentationContent = `
  <h1>Ma présentation</h1>
    <div class="glass detailed-content" style="padding: 3rem; margin-top: 2rem;">
      <h3>Présentation générale</h3>
      <p>De nature <b>curieuse</b>, j’ai toujours eu une envie d’<b>apprendre</b> dans des domaines en tout genre. Cette soif de connaissances me pousse à toujours essayer de <b>repousser mes limites</b>. J’aime aussi
        travailler sur des projets utilisant de nouvelles technologies et relever de nouveaux défis toujours plus <b>complexes</b> et <b>ambitieux</b>. Cela me permet de me remettre en question constamment et de
        régulièrement progresser.
      </p>
      <p>Actuellement étudiant de 5ème année à l’<b>ESIEA</b> dans la filière Administrateur des Systèmes d’Informations (ASI), et alternant en tant qu’Administrateur d’Infrastructures au sein de l’entreprise
        <b>Berger-Levrault</b>, j’ai eu l’occasion de travailler sur de nombreux projets variés. Ces expériences m’ont permis de découvrir mes préférences dans ce métier : les <b>architectures réseaux</b> et les
        <b>infrastructures virtualisées</b>.
      </p>
      <p>Fort d’une <b>expérience pluriannuelle</b> au sein des équipes de gestion d’un datacenter, j’ai appris à m’adapter à des infrastructures de grande taille, et à la nécessité de livrer des solutions
        techniques conformes à diverses <b>réglementations</b> telles que l’<b>ISO 27001</b> ou la <b>certification HDS</b>. Le large éventail de technologies que j’ai pu utiliser me permettent de créer et gérer des
        <b>infrastructures sécurisées et redondantes</b>.
      </p>

      <h3>Mes valeurs</h3>
      <p>Mes valeurs sont l’<b>esprit d’équipe</b>, l’<b>investissement personnel</b> et la <b>rigueur</b>.</p>
      <p>
        Dans le cadre des réalisations <b>DockStock</b> et <b>Walltesters</b> pendant mon <b>Bachelor AIS</b>, j’ai appris à travailler avec mes camarades pour réaliser des projets toujours plus 
        complexes et ambitieux. J’ai beaucoup apprécié de pouvoir <b>défendre mes idées techniques</b> avec eux et de <b>raisonner</b> afin d’appliquer les choix technologiques les plus pertinents pour réussir 
        mes projets.
      </p>
      <p>
        Je me suis énormément <b>investi</b> dans mes études afin d’acquérir toutes les compétences que j’ai aujourd’hui. Ce bagage aussi bien <b>technique</b> (systèmes, réseaux, cybersécurité, etc.) 
        qu’<b>humain</b> (relationnel, organisationnel, humain, etc.) est le fruit d’un travail constant et important. J’ai pu l’appliquer dans le domaine professionnel pendant mon alternance chez 
        <b>Berger-Levrault</b>, me permettant de maîtriser l’infrastructure complexe d’un datacenter, ainsi que les nombreuses technologies diverses qui le composent.
      </p>
      <p>
        La <b>rigueur</b> est aussi une valeur que j’applique dans mon travail. J’apprécie réaliser des <b>solutions propres et complètes</b>, et souhaite partager un travail de qualité à mes collègues. 
        Lors de mon alternance chez <b>Berger-Levrault</b>, j’ai eu l’occasion de travailler sur une infrastructure conforme à des <b>réglementations</b> et <b>constamment auditée</b>. Cela m’a appris à rédiger des 
        documentations exhaustives, ainsi que des timelines d’opération et des comptes-rendus de toutes les actions que je réalise.
      </p>

      <h3>Mes qualités humaines</h3>
      <p>Je suis une personne plutôt <b>motivée</b>, <b>persévérante</b> et <b>autonome</b>.</p>
      <p>
        Que ce soit dans le cadre privé ou professionnel, j’essaie toujours d’être le plus <b>motivé</b> possible. Dans un secteur qui évolue aussi vite que l'informatique, je considère chaque nouveau 
        défi technique comme une opportunité de <b>monter en compétences</b>. C'est cette envie de progresser qui me pousse à m'investir pleinement dans la réussite des projets, afin de rendre un 
        travail dont la qualité ne cesse de s’améliorer.
      </p>
      <p>
        J’ai également appliqué ma <b>persévérance</b> chez <b>Berger-Levrault</b> en tant qu’<b>Administrateur d’Infrastructures</b>. Dans le cadre de ce métier, je suis appelé à maintenir les 
        infrastructures aussi bien physiques que réseau d’un <b>datacenter</b>. Lors de projets complexes ou d’incidents de production, je dois faire preuve de persévérance car certains de ces 
        derniers me demandent plusieurs heures avant de pouvoir effectuer un diagnostic précis et trouver des solutions. La patience dont je fais preuve au quotidien me permet de mener à bout ces 
        investigations et d’accomplir les missions qui me sont confiées.
      </p>
      <p>
        Mon aptitude à être <b>autonome</b> a toujours été <b>déterminante</b> dans ma scolarité. Lors de mes études comme dans mon métier, j’ai pu mettre en place de nombreux logiciels, rédiger des 
        documentations techniques, réaliser des infrastructures de test (POC) et résoudre des incidents divers en toute <b>autonomie</b>. Cette dernière m’a permis de maîtriser de nombreuses 
        technologies et concepts rapidement et d’accomplir mes missions.
      </p>

      <h3>Le plan personnel</h3>
      <p>Au-delà du monde de l’informatique, je suis quelqu’un de <b>discret</b> et <b>curieux</b>. Lorsque je m’intéresse à un sujet, j’aime <b>approfondir mes connaissances</b> dessus autant que 
        possible. Je suis fortement intéressé par les <b>sports mécaniques</b> depuis mon enfance, qui sont des disciplines de pointe extrêmement compétitives, ainsi que les <b>jeux vidéo</b>, qui sont des sources
        d’inspiration et de créativité pour moi. Je suis aussi un grand passionné <b>d’histoire</b>, notamment de la période des Guerres Mondiales et de la Guerre Froide, car les évolutions de la société et des
        technologies pendant cette période me fascinent.
      </p>

      <h3>Mon projet professionnel</h3>
      <p>
        J’ai pour projet professionnel d’exercer en tant qu’<b>architecte réseau</b> pour une grande entreprise. C’est un métier qui consiste en la conception et la planification de réseaux de 
        grande taille. C’est une évolution de mon métier actuel, demandant une grande maîtrise technique et une capacité analytique et de conception.
      </p>
      <p>Mes <b>projets personnels</b> sont de développer mon propre jeu vidéo et le proposer au grand public, ainsi que de renforcer ma <b>culture générale</b>, notamment en apprenant <b>l’allemand</b>.</p>
    </div>
`;
